...
\begin{equation}
\hat{\theta} = \displaystyle{\mathop{argmax}\mathbb{E}_{x \sim \mathcal{D}_e(x), y \sim g(x)}}log P(y|x;\theta),
\label{eq:mle-chap2}
\end{equation}
...
\hat{\theta} &=& \displaystyle{\mathop{argmin}} \ L_{CE}(\theta,\mathcal{D}_e,g).
\end{array}
\label{eq:ce-chap2}
\end{equation}
...
\begin{equation}
\begin{array}{rcl}
L_{CE}(\theta, \mathcal{D}_e, g) &=& -\displaystyle{ \mathbb{E}_{x \sim \mathcal{D}_e(x), y \sim g(x)} \big[ \mathop{\sum}_{i}^{l_y}} \big( (1-\epsilon) * log P(y_i|y_{<i},x;\theta) + \frac{\epsilon}{\mid \Sigma_y \mid} * \displaystyle{\mathop{\sum}_{y' \in \Sigma_y}} log P(y = y'|y_{<i},x;\theta)\big) \big], \\
\end{array}
\end{equation}
